parse_2019_contijoch_elife_5data16S <- function() {
  ...
  library(tidyverse)
    ...
    local               <- "2019_contijoch_elife_multispeciesqPCRshotgunandamplicon/"
    repro_counts_rds_zip<- paste0(local, "PRJNA413199_dada2_merged_nochim.rds.zip")
    repro_tax_zip       <- paste0(local, "PRJNA413199_dada2_taxonomy_merged.rds.zip")
    motus_zip           <- paste0(local, "PRJNA413199_motus_merged.tsv.zip")
    metaphlan4_zip      <- paste0(local, "PRJNA413199_MetaPhlAn_merged.tsv.zip")
    scale_16s_zip     <- paste0(local, "Contijoch2019 16S_scale.csv.zip")
    scale_meta_zip    <- paste0(local, "Contijoch2019_scale.csv.zip")
    counts_16s_zip    <- paste0(local, "Contijoch_2019_16S.csv.zip")
    metadata_16s_zip  <- paste0(local, "Contijoch_2019_metadata.csv.zip") 
    counts_meta_zip   <- paste0(local, "Contijoch_2019_shotgunmetagenomics.csv.zip")
    metadata_meta_zip <- paste0(local, "Contijoch_metadata.csv.zip")

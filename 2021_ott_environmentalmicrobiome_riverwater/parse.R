parse_2021_ott_environmentalmicrobiome_riverwater <- function(raw = FALSE, align = FALSE) {
  ...
  if (all(file.exists(repro_counts_rds_zip), file.exists(repro_tax_zip))) {
    ...
    if (!raw) {
        ...
        counts_reprocessed2 = aligned_counts$reprocessed
  ...
  return(list(
    counts      = list(original = NA, reprocessed = list(rdp19 = counts_reprocessed, rdp16 = counts_reprocessed2)),
    proportions = list(original = NA, reprocessed = list(rdp19 = proportions_reprocessed, rdp16 = proportions_reprocessed2)),
    tax         = list(original = NA, reprocessed = list(rdp19 = tax_reprocessed, rdp16 = tax_reprocessed2)),
    scale       = scale,
    metadata    = metadata
  ))
}
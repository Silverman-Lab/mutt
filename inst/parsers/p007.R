parse_2018_piwosz_ismej_spikeinampliconfreshwater <- function(raw = FALSE, align = FALSE) {
    ...
    # ---- metadata ----
    metadata = NA
    metadata = read_zipped_table(metadata_SRA_zip) %>% rename(Accession = Run)

    tax_original = NA
    proportions_original = NA

    # ---- original tax, counts, and proportions ----
    counts_original = read_zipped_table(taxcounts_zip)
    if (!raw) {
    ...
    tax_original = colnames(counts_original)
    ...
    if (!raw) {
      ...
      tax_reprocessed2 = fill_na_zero_numeric(tax_reprocessed2)
    }

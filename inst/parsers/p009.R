parse_2018_tourlousse_scientificreports_soilsewagespikein <- function(raw = FALSE, align = FALSE) {
    ...
    counts_zip           <- file.path(local, "")
    metadata_zip         <- file.path(local, "")
    metadata_SRA_zip     <- file.path(local, "SraRunTable (38).csv.zip")

    # ---- original scounts, tax, and proportions ----
    original_counts = NA
    tax_original = NA
    ...
    if (all(file.exists(repro_counts_rds_zip), file.exists(repro_tax_zip))) {
        ...
        if (!raw) {
            aligned = rename_and_align(counts_reprocessed = counts_reprocessed, metadata, scale, by_col = "ID", align = align, study_name = basename(local))

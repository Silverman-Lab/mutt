parse_2022_Cvandevelde_ismecommunications_culturedflowhumanfecal <- function() {
  ...
  repro_counts_rds_zip<- paste0(local, "PRJEB51873_dada2_merged_nochim.rds.zip")
  repro_tax_zip       <- paste0(local, "PRJEB51873_dada2_taxonomy_merged.rds.zip")
  scale_16s_zip     <- paste0(local, "VandeVelde2022_scale.csv.zip")
  counts_16s_zip    <- paste0(local, "VandeVelde_2022_16S.csv.zip")
  metadata_16s_zip  <- paste0(local, "VandeVelde_2022_metadata.csv.zip") 
